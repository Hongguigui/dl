...
def evaluate_model(x, y, model, name):
    ...
    with open('log.txt', 'a') as f:
        f.write(f"{name}_loss: " + str(eval_loss.numpy()) + '\n')
        f.write(f"{name}_acc: " + str(eval_accuracy.numpy()) + '\n')
...
if __name__ == '__main__':
    ...
    classifier = Classifier(input_depth=x_train.shape[-1], layer_depths=[2, 1], layer_kernel_sizes=kernels,
                            num_classes=10, hidden_width=64, input_dim=x_train[0].shape, num_channels=[4, 8])
    ...
    evaluate_model(x_train, y_train, classifier, 'train')
